...
for DEGREE in $DEGREES; do
    ...
    #         echo -e $FILENAME_CCLUSTER_OUT
            $CCLUSTER_CALL $DEGREE $BITSIZE $BINIT $EPSIL $STRAT > $FILENAME_CCLUSTER_OUT
        ...
        if [ $MPSOLVE -eq $TRUE ]; then
            ...
            FILENAME_MPSOLVE_IN=$FILE_PATH"/"$POLY_NAME"_"$DEGREE"_"$PRECISIONS".pol"
            FILENAME_MPSOLVE_OUT=$FILE_PATH"/"$POLY_NAME"_"$DEGREE"_"$PRECISIONS"_mpsolve.out"
            $GENPOLFILE_CALL $DEGREE $PRECISIONS > $FILENAME_MPSOLVE_IN
            (time $MPSOLVE_CALL $FILENAME_MPSOLVE_IN > $FILENAME_MPSOLVE_OUT) &>> $FILENAME_MPSOLVE_OUT
            $MPSOLVE_CALL -Ogf $FILENAME_MPSOLVE_IN | gnuplot
        ...
        if [ $MPSOLVE_S -eq $TRUE ]; then    
            ...
            FILENAME_MPSOLVE_IN=$FILE_PATH"/"$POLY_NAME"_"$DEGREE"_"$PRECISIONS".pol"
            FILENAME_MPSOLVE_S_OUT=$FILE_PATH"/"$POLY_NAME"_"$DEGREE"_"$PRECISIONS"_mpsolve_s.out"
            $GENPOLFILE_CALL $DEGREE $PRECISIONS > $FILENAME_MPSOLVE_IN
