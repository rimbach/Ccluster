...
/* memory managment */
static __inline__ void compRat_init(compRat_t x) { 
    realRat_init(compRat_realref(x)); 
    realRat_init(compRat_imagref(x));
}

static __inline__ void compRat_clear(compRat_t x) { 
...
/* members access */
static __inline__ realRat_ptr compRat_real_ptr(compRat_t x) {
    return compRat_realref(x);
}

static __inline__ realRat_ptr compRat_imag_ptr(compRat_t x) {
    return compRat_imagref(x);
}

static __inline__ void compRat_get_real(realRat_t re, const compRat_t x) {
    realRat_set(re, compRat_realref(x));
}

static __inline__ void compRat_get_imag(realRat_t im, const compRat_t x) {
    realRat_set(im, compRat_imagref(x));
}

/* setting */
static __inline__ void compRat_set_2realRat(compRat_t x, const realRat_t re, const realRat_t im) { 
    realRat_set(compRat_realref(x), re); 
    realRat_set(compRat_imagref(x), im);
}

static __inline__ void compRat_set_sisi(compRat_t x, slong preal, ulong qreal, slong pimag, ulong qimag) { 
    realRat_set_si(compRat_realref(x), preal, qreal); 
    realRat_set_si(compRat_imagref(x), pimag, qimag);
}

static __inline__ int compRat_set_str  (compRat_t x, const char * strReN, const char * strReD, const char * strImN, const char * strImD, int b){
...
static __inline__ void compRat_set(compRat_t dest, const compRat_t src) { 
...
static __inline__ void compRat_print(const compRat_t x) {
    return compRat_fprint(stdout, x);
}
