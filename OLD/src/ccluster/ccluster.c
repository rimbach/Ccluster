...
void connCmp_print_for_results(FILE * f, const connCmp_t c, slong prec, slong digits, metadatas_t meta){
    ...
    fprintf(f, "center: ");
    realApp_fprintn(f, cRe, digits, ARB_STR_NO_RADIUS);
    fprintf(f, " + ");
    realApp_fprintn(f, cIm, digits, ARB_STR_NO_RADIUS);
    fprintf(f, "j, radius: ");
    realApp_fprintn(f, rad, digits, ARB_STR_NO_RADIUS);
...
void connCmp_list_print_for_results(FILE * f, const connCmp_list_t l, slong prec, slong digits, metadatas_t meta){
    connCmp_list_iterator it = connCmp_list_begin(l);
    while (it!=connCmp_list_end() ) {
        fprintf(f, " cluster with %5d sols: ", connCmp_nSols(connCmp_list_elmt(it)));
        connCmp_print_for_results(f, connCmp_list_elmt(it), prec, digits, meta);
        it = connCmp_list_next(it);
        fprintf(f, "\n");
    }
}
...
void ccluster_interface_func( void(*func)(compApp_poly_t, slong), const compBox_t initialBox, const realRat_t eps, int st, int verb){
    ...
    if (verb>=3) {
        connCmp_list_print_for_results(stdout, qRes, 53, 16, meta);
...
void ccluster_interface_forJulia( connCmp_list_t qResults, 
    ...
    if (verb>=3) {
        connCmp_list_print_for_results(stdout, qResults, 53, 16, meta);
