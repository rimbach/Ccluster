...
void getApprox_Zero(compApp_poly_t dest, slong prec);
...
int main(int argc, char **argv){
    ...
    if (parse) {
       if (global==2)
            ccluster_global_interface_func( getApprox_Zero, eps, st, nbthreads, output, verbosity);
       else
            ccluster_interface_func( getApprox_Zero, bInit, eps, st, nbthreads, output, verbosity);
