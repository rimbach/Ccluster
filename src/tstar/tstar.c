...
void tstar_graeffe_iterations_abs_two_first_coeffs( realApp_t coeff0, realApp_t coeff1, const compApp_poly_t pApprox, int N, slong prec, metadatas_t meta){
    ...
    compApp_poly_init(p1);
    compApp_poly_init(p2);
...
tstar_res tstar_asInPaper( cacheApp_t cache,
    ...
    metadatas_add_Test( meta, depth, (restemp!=0), discard, nbTaylorsRepeted, N, nbGraeffeRepeted);
...
//         tstar_inclusion_test( cache, d, 53, depth, meta);
        if (tstar_inclusion_test( cache, d, 53, depth, meta) == 1)
            restemptemp = -1;
    ...
    if ((discard)&&( CCLUSTER_EXP_NUM_T0(meta)||CCLUSTER_EXP_NUM_T1(meta) ) ) {
        ...
        int test = tstar_numerical_test( pApprox, d, res.appPrec, meta);
        
        if ((test==0)&&CCLUSTER_EXP_NUM_T0(meta)) {
            compApp_poly_clear(pApprox);
            realApp_clear(sum);
            res.nbOfSol = 0;
            metadatas_add_Test( meta, depth, 1, discard, 0, 0, 0);
            printf("--depth: %d, success of C0 predicate\n", depth);
            return res;
        }
        if ((test==1)&&CCLUSTER_EXP_NUM_T1(meta)) {
            compApp_poly_clear(pApprox);
            realApp_clear(sum);
            res.nbOfSol = 1;
            metadatas_add_Test( meta, depth, 1, discard, 0, 0, 0);
            printf("--depth: %d, success of numerical T1Test \n", depth);
            return res;
        }
    }
    if (restemp==0) {
    ...
    while( (iteration <= N)&&(restemp==0) ){
        ...
        if ( (discard) && (metadatas_useAnticipate(meta)) && (anticipate_already_applied==0) && (restemp == 0) ) {
            ...
            if (test_anticipate) {
                ...
                compApp_abs( coeffn, compApp_poly_getCoeff(pApprox, compApp_poly_degree(pApprox)), res.appPrec);
                realApp_pow_ui( coeffn, coeffn, (ulong)(0x1 << N), res.appPrec);
    ...
    metadatas_add_Test( meta, depth, (restemp==1), discard, nbTaylorsRepeted, nbGraeffe, nbGraeffeRepeted);
...
//     }
    ...
    if ( (restemptemp==-1) ) {
        printf("**********************************restemptemp: %d, res.nbOfSol: %d \n", restemptemp, res.nbOfSol);
    }
