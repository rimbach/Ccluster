...
/* properties */
static __inline__ int connCmp_is_empty(const connCmp_t x){
    return compBox_list_is_empty(connCmp_boxesref(x));
}

static __inline__ int connCmp_nb_boxes(const connCmp_t x){
    return compBox_list_get_size(connCmp_boxesref(x));
}


static __inline__ void connCmp_width(realRat_t dest, connCmp_t x){
    realRat_set(dest, connCmp_widthref(x));
}


static __inline__ void connCmp_infRe(realRat_t dest, connCmp_t x){
    realRat_set(dest, connCmp_infReref(x));
}


static __inline__ void connCmp_supRe(realRat_t dest, connCmp_t x){
    realRat_set(dest, connCmp_supReref(x));
}


static __inline__ void connCmp_infIm(realRat_t dest, connCmp_t x){
    realRat_set(dest, connCmp_infImref(x));
}


static __inline__ void connCmp_supIm(realRat_t dest, connCmp_t x){
...
static __inline__ void connCmp_nwSpd(fmpz_t dest, connCmp_t x){
...
static __inline__ void connCmp_print(const connCmp_t x) {
    return connCmp_fprint(stdout, x);
}
...
static __inline__ compBox_ptr connCmp_pop(connCmp_t x) {
    return compBox_list_pop( connCmp_boxesref(x) );
}
/* do not maintain connectedness nor hull */
static __inline__ void connCmp_push(connCmp_t x, compBox_t b) {
    compBox_list_push( connCmp_boxesref(x), b );
}

/* do not check bound!!! */
static __inline__ compBox_ptr connCmp_compBox_at_index(connCmp_t x, int index) {
