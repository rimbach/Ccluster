...
// void connCmp_set_forjulia(connCmp_t dest, const connCmp_t src){
...
int connCmp_is_empty_for_julia(const connCmp_t x){
    return compBox_list_is_empty(connCmp_boxesref(x));
}
void connCmp_compBox_at_index_for_julia(compBox_t dest, connCmp_t x, int index) {
//     printf("connCmp_compBox_at_index; index: %d, len: %d\n", index, connCmp_nb_boxes(x) );
//     compBox_print( compBox_list_compBox_at_index( connCmp_boxesref(x), index )); printf("\n");
    compBox_set(dest, compBox_list_compBox_at_index( connCmp_boxesref(x), index ));
}
int connCmp_nb_boxes_for_julia(const connCmp_t x){
    return compBox_list_get_size(connCmp_boxesref(x));
}
void connCmp_width_for_julia(realRat_t dest, connCmp_t x){
    realRat_set(dest, connCmp_widthref(x));
}
void connCmp_infRe_for_julia(realRat_t dest, connCmp_t x){
    realRat_set(dest, connCmp_infReref(x));
}
void connCmp_supRe_for_julia(realRat_t dest, connCmp_t x){
    realRat_set(dest, connCmp_supReref(x));
}
void connCmp_infIm_for_julia(realRat_t dest, connCmp_t x){
    realRat_set(dest, connCmp_infImref(x));
}
void connCmp_supIm_for_julia(realRat_t dest, connCmp_t x){
    realRat_set(dest, connCmp_supImref(x));
}
void connCmp_nwSpd_for_julia(fmpz_t dest, connCmp_t x){
    fmpz_set(dest, connCmp_nwSpdref(x));
}
void connCmp_set_nwSpd_for_julia(connCmp_t x, fmpz_t spd){
    fmpz_set(connCmp_nwSpdref(x), spd);
}

void connCmp_pop_for_julia(compBox_t dest, connCmp_t x) {
//     compBox_ptr bn = compBox_list_pop( connCmp_boxesref(x) );
//     compBox_set(dest, bn);
//     compBox_clear(bn);
//     printf("pop; len: %d ",connCmp_nb_boxes(x)); compBox_print( compBox_list_first( connCmp_boxesref(x) ) ); printf("\n");
    
    compBox_set(dest, compBox_list_pop( connCmp_boxesref(x) ));
}
void connCmp_push_for_julia(connCmp_t x, compBox_t b) {
//     compBox_t bn;
//     compBox_init(bn);
//     compBox_set(bn, b);
//     compBox_list_push( connCmp_boxesref(x), bn );
    compBox_list_push( connCmp_boxesref(x), b  );
}

void connCmp_insert_compBox_for_julia(connCmp_t x, compBox_t b){
//     printf("----------------insert:---------------- \n");
//     printf("b: "); compBox_print(b); printf("\n");
//     compBox_t bn;
//     compBox_init(bn);
//     compBox_set(bn, b);
//     printf("bn: "); compBox_print(bn); printf("\n");
//     compBox_list_print(connCmp_boxesref(x));printf("\n");
//     connCmp_insert_compBox(x, bn);
    connCmp_insert_compBox(x, b);
//     for (int i=0; i< connCmp_nb_boxes(x); i++){
//         compBox_print( connCmp_compBox_at_index(x, i) ); printf("\n");
//     }
//     compBox_list_print(connCmp_boxesref(x));printf("\n");
//     printf("------------------------------------- \n");
//     connCmp_print(x); printf("\n");
//     connCmp_insert_compBox(x, b);
}
