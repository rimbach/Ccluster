...
/* as a consequence, if a cc intersects aunique annulus of the first list that contains one root, */
...
void risolate_discard_compBox_list_prepLoop( compBox_list_t boxes, 
...
//         printf("nbMSol: %d\n", (int) compBox_get_nbMSol(btemp) );
        ...
        if ( metadatas_useRootRadii(meta) ){
            ...
            if ( risolate_compBox_intersects_atLest_one( btemp, 2 ) == 0 ){
                if (metadatas_haveToCount(meta)){
                    metadatas_add_discarded( meta, depth);
                }
//                 if (metadatas_getDrSub(meta)==0){
...
void risolate_bisect_connCmp_prepLoop( connCmp_list_t dest, 
                                       connCmp_t cc, 
                                       connCmp_list_t discardedCcs,
                                       compBox_list_t bDiscarded, 
                                       cacheApp_t cache, 
    ...
    risolate_discard_compBox_list_prepLoop(subBoxes, bDiscarded, cache, prec, meta);
...
void risolate_prep_loop_rootRadii_n( compBox_list_t bDiscarded, 
    ...
    while (!connCmp_list_is_empty(qPrepLoop)) {
        ...
        if (intersectOnlyOneFlag == 0) {
            risolate_bisect_connCmp_prepLoop( ltemp, ctemp, discardedCcs, bDiscarded, cache, meta, metadatas_useNBThreads(meta));
            while (!connCmp_list_is_empty(ltemp))
...
//                     
                    ...
                    compBox_nbMSolref( nbox ) = risolate_compBox_nbMsols( bcur, 2 );
                    /* copy annulii 0 */
                    compBox_copy_annulii(nbox, 0, compBox_annuliref(bcur, 0));
                    /* copy annulii 1 */
//                     connCmp_mergeAnnulii(compBox_annuliref(nbox, 1), 1, ctemp);
                    /*delete ctemp*/
                    connCmp_clear(ctemp);
                    ccluster_free(ctemp);
...
//                if ( metadatas_useRootRadii(meta) ){
//                     int containsOnlyOne = risolate_connCmp_intersects_only_one( ccur, 2 );
//                     printf("# contains only one : %d, contains one real: %d\n", containsOnlyOne, risolate_connCmp_ContainsOneRealRoot( ccur, 2 ));
//                 }
//            }
//         if ((separationFlag)) {
...
//     prec = realIntRootRadii_rootRadii( annulii2, centerRe2, cache, delta );
    ...
    //     if (metadatas_getVerbo(meta)>3) {
//         printf("#time in computing 2nd RootRadii           : %f \n", ((double) (clock() - start2))/CLOCKS_PER_SEC );
//         printf("#precision needed: %ld\n", prec);
//     }
    start2 = clock();
...
//     compBox_copy_annulii(box, 0, annulii1);
//     compBox_copy_annulii(box, 1, annulii);
//     compBox_copy_annulii(box, 2, annulii2);
    ...
    connCmp_list_push(qPrepLoop, initialCC);
    risolate_prep_loop_rootRadii_n(bDiscarded, qMainLoop, qPrepLoop, discardedCcs, cache, meta);
