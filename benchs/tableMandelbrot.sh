...
usage()
{
   echo "Usage: ./tableMandelbrot <options> <args>"
}
...
##########################solvers
CCLUSTER_PATH="../"
CCLUSTER_CALL=$CCLUSTER_PATH"/bin/MACIS19/ccluster"
...
#     LINE_TAB=$DEG
    ...
    if [ ! -e $FILENAME_CCLUSTER_L_OUT ]; then
        ...
        $CCLUSTER_CALL"_mandelbrot" $DEG -d $BLOCAL -e $EPSILONCCL -m $VFLAG -v 3 > $FILENAME_CCLUSTER_L_OUT
    fi
    if [ ! -e $FILENAME_CCLUSTER_G_OUT ]; then
        ...
        $CCLUSTER_CALL"_mandelbrot" $DEG -e $EPSILONCCL -m $VFLAG -v 3 > $FILENAME_CCLUSTER_G_OUT
    fi
