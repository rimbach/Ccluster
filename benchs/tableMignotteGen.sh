...
CCLUSTER_CALL="../test/mignotteGen"
    ...
    for BIT in $BITSIZES; do
        for POW in $POWERS; do
            ...
            if [ ! -e $FILENAME_CCLUSTER_L_OUT ]; then
                ...
                $CCLUSTER_CALL $DEG $BIT $POW $BLOCAL $EPSILONCCL $VFLAG "3" > $FILENAME_CCLUSTER_L_OUT
            fi
            if [ ! -e $FILENAME_CCLUSTER_G_OUT ]; then
                ...
                $CCLUSTER_CALL $DEG $POW $BIT $BGLOBAL $EPSILONCCL $VFLAG "3" > $FILENAME_CCLUSTER_G_OUT
            fi
            $GENPOLFILE_CALL $DEG $BIT $POW > $FILENAME_MPSOLVE_IN
