...
##########################solvers
SOLVER_PATH="./"
MPSOLVE_CALL="mpsolve -au -Gi -Of -o"$EPSILONMPS" -j1"
MPSOLVE_CALL_S="mpsolve -as -Gi -Of -o"$EPSILONMPS" -j1"

##########################naming
POLY_NAME="MignotteGen"
...
for DEGREE in $DEGREES; do
    ...
    for POWER in $POWERS; do
        if [ $CCLUSTER -eq $TRUE ]; then
            ...
            FILENAME_CCLUSTER_OUT=$FILE_PATH"/"$POLY_NAME"_"$DEGREE"_"$BITSIZE"_"$POWER"_ccluster.out"
    #         echo -e $FILENAME_CCLUSTER_OUT
            $CCLUSTER_CALL $DEGREE $BITSIZE $POWER $BINIT $EPSIL $STRAT "3" > $FILENAME_CCLUSTER_OUT
        ...
        if [ $MPSOLVE -eq $TRUE ]; then
            ...
            FILENAME_MPSOLVE_IN=$FILE_PATH"/"$POLY_NAME"_"$DEGREE"_"$BITSIZE"_"$POWER".pol"
            FILENAME_MPSOLVE_OUT=$FILE_PATH"/"$POLY_NAME"_"$DEGREE"_"$BITSIZE"_"$POWER"_mpsolve.out"
            $GENPOLFILE_CALL $DEGREE $BITSIZE $POWER > $FILENAME_MPSOLVE_IN
        ...
        if [ $MPSOLVE_S -eq $TRUE ]; then    
            ...
            FILENAME_MPSOLVE_IN=$FILE_PATH"/"$POLY_NAME"_"$DEGREE"_"$BITSIZE"_"$POWER".pol"
            FILENAME_MPSOLVE_S_OUT=$FILE_PATH"/"$POLY_NAME"_"$DEGREE"_"$BITSIZE"_"$POWER"_mpsolve_s.out"
            $GENPOLFILE_CALL $DEGREE $BITSIZE $POWER > $FILENAME_MPSOLVE_IN
